...
public class TerrainFaceNeptune {

    // Shape Generator
    ShapeGeneratorNeptune shapeGeneratorNeptune;
    ...
    public TerrainFaceNeptune(ShapeGeneratorNeptune shapeGeneratorNeptune, Mesh mesh, int resolution, Vector3 localUp) {

        // Shape generator
        this.shapeGeneratorNeptune = shapeGeneratorNeptune;
    ...
    public void ConstructMeshNeptune() {
        ...
        for (int y = 0; y < resolution; y++) {
            ...
            for (int x = 0; x < resolution; x++) {
                ...
                Vector3 pointOnUnitCubeNeptune = localUp + (percent.x - 0.5f) * 2 * axisA + (percent.y - 0.5f) * 2 * axisB;

                // vector 3 for point on unit sphere
                Vector3 pointOnUnitSphereNeptune = pointOnUnitCubeNeptune.normalized;

                // Unscaled elevation = shape generator to calculate unscaled elevation with point on unit sphere
                float unscaledElevationNeptune = shapeGeneratorNeptune.CalculateUnscaledElevationNeptune(pointOnUnitSphereNeptune);

                // verrticies with index of i = point on unit sphere * shape generator with unscaled elevation
                vertices[i] = pointOnUnitSphereNeptune * shapeGeneratorNeptune.GetScaledElevationNeptune(unscaledElevationNeptune);

                // uv's on the y axis = unscaled elevation
                uv[i].y = unscaledElevationNeptune;
    ...
    // Update UV's
    public void UpdateUVsNeptune(ColourGeneratorNeptune colourGeneratorNeptune) {
        ...
        for (int y = 0; y < resolution; y++) {
            ...
            for (int x = 0; x < resolution; x++) {
                ...
                uv[i].x = colourGeneratorNeptune.BiomePercentFromPointNeptune(pointOnUnitSphereNeptune);
