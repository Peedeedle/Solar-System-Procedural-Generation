...
public class TerrainFaceSaturn {

    // Shape Generator
    ShapeGeneratorSaturn shapeGeneratorSaturn;
    ...
    public TerrainFaceSaturn(ShapeGeneratorSaturn shapeGeneratorSaturn, Mesh mesh, int resolution, Vector3 localUp) {

        // Shape generator
        this.shapeGeneratorSaturn = shapeGeneratorSaturn;
    ...
    public void ConstructMeshSaturn() {
        ...
        for (int y = 0; y < resolution; y++) {
            ...
            for (int x = 0; x < resolution; x++) {
                ...
                Vector3 pointOnUnitCubeSaturn = localUp + (percent.x - 0.5f) * 2 * axisA + (percent.y - 0.5f) * 2 * axisB;

                // vector 3 for point on unit sphere
                Vector3 pointOnUnitSphereSaturn = pointOnUnitCubeSaturn.normalized;

                // Unscaled elevation = shape generator to calculate unscaled elevation with point on unit sphere
                float unscaledElevationSaturn = shapeGeneratorSaturn.CalculateUnscaledElevationSaturn(pointOnUnitSphereSaturn);

                // verrticies with index of i = point on unit sphere * shape generator with unscaled elevation
                vertices[i] = pointOnUnitSphereSaturn * shapeGeneratorSaturn.GetScaledElevationSaturn(unscaledElevationSaturn);

                // uv's on the y axis = unscaled elevation
                uv[i].y = unscaledElevationSaturn;
    ...
    // Update UV's
    public void UpdateUVsSaturn(ColourGeneratorSaturn colourGeneratorSaturn) {
        ...
        for (int y = 0; y < resolution; y++) {
            ...
            for (int x = 0; x < resolution; x++) {
                ...
                uv[i].x = colourGeneratorSaturn.BiomePercentFromPointSaturn(pointOnUnitSphereSaturn);
