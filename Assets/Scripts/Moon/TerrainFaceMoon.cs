...
public class TerrainFaceMoon {

    // Shape Generator
    ShapeGeneratorMoon shapeGeneratorMoon;
    ...
    public TerrainFaceMoon(ShapeGeneratorMoon shapeGeneratorMoon, Mesh mesh, int resolution, Vector3 localUp) {

        // Shape generator
        this.shapeGeneratorMoon = shapeGeneratorMoon;
    ...
    public void ConstructMeshMoon() {
        ...
        for (int y = 0; y < resolution; y++) {
            ...
            for (int x = 0; x < resolution; x++) {
                ...
                Vector3 pointOnUnitCubeMoon = localUp + (percent.x - 0.5f) * 2 * axisA + (percent.y - 0.5f) * 2 * axisB;

                // vector 3 for point on unit sphere
                Vector3 pointOnUnitSphereMoon = pointOnUnitCubeMoon.normalized;

                // Unscaled elevation = shape generator to calculate unscaled elevation with point on unit sphere
                float unscaledElevationMoon = shapeGeneratorMoon.CalculateUnscaledElevationMoon(pointOnUnitSphereMoon);

                // verrticies with index of i = point on unit sphere * shape generator with unscaled elevation
                vertices[i] = pointOnUnitSphereMoon * shapeGeneratorMoon.GetScaledElevationMoon(unscaledElevationMoon);

                // uv's on the y axis = unscaled elevation
                uv[i].y = unscaledElevationMoon;
    ...
    // Update UV's
    public void UpdateUVsMoon(ColourGeneratorMoon colourGeneratorMoon) {
        ...
        for (int y = 0; y < resolution; y++) {
            ...
            for (int x = 0; x < resolution; x++) {
                ...
                uv[i].x = colourGeneratorMoon.BiomePercentFromPointMoon(pointOnUnitSphereMoon);
