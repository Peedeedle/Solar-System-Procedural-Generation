...
public class TerrainFaceMars {

    // Shape Generator
    ShapeGeneratorMars shapeGeneratorMars;
    ...
    public TerrainFaceMars(ShapeGeneratorMars shapeGeneratorMars, Mesh mesh, int resolution, Vector3 localUp) {

        // Shape generator
        this.shapeGeneratorMars = shapeGeneratorMars;
    ...
    public void ConstructMeshMars() {
        ...
        for (int y = 0; y < resolution; y++) {
            ...
            for (int x = 0; x < resolution; x++) {
                ...
                Vector3 pointOnUnitCubeMars = localUp + (percent.x - 0.5f) * 2 * axisA + (percent.y - 0.5f) * 2 * axisB;

                // vector 3 for point on unit sphere
                Vector3 pointOnUnitSphereMars = pointOnUnitCubeMars.normalized;

                // Unscaled elevation = shape generator to calculate unscaled elevation with point on unit sphere
                float unscaledElevationMars = shapeGeneratorMars.CalculateUnscaledElevationMars(pointOnUnitSphereMars);

                // verrticies with index of i = point on unit sphere * shape generator with unscaled elevation
                vertices[i] = pointOnUnitSphereMars * shapeGeneratorMars.GetScaledElevationMars(unscaledElevationMars);

                // uv's on the y axis = unscaled elevation
                uv[i].y = unscaledElevationMars;
    ...
    // Update UV's
    public void UpdateUVsMars(ColourGeneratorMars colourGeneratorMars) {
        ...
        for (int y = 0; y < resolution; y++) {
            ...
            for (int x = 0; x < resolution; x++) {
                ...
                uv[i].x = colourGeneratorMars.BiomePercentFromPointMars(pointOnUnitSphereMars);
