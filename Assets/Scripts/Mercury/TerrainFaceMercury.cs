...
public class TerrainFaceMercury {

    // Shape Generator
    ShapeGeneratorMercury shapeGeneratorMercury;
    ...
    public TerrainFaceMercury(ShapeGeneratorMercury shapeGeneratorMercury, Mesh mesh, int resolution, Vector3 localUp) {

        // Shape generator
        this.shapeGeneratorMercury = shapeGeneratorMercury;
    ...
    public void ConstructMeshMercury() {
        ...
        for (int y = 0; y < resolution; y++) {
            ...
            for (int x = 0; x < resolution; x++) {
                ...
                Vector3 pointOnUnitCubeMercury = localUp + (percent.x - 0.5f) * 2 * axisA + (percent.y - 0.5f) * 2 * axisB;

                // vector 3 for point on unit sphere
                Vector3 pointOnUnitSphereMercury = pointOnUnitCubeMercury.normalized;

                // Unscaled elevation = shape generator to calculate unscaled elevation with point on unit sphere
                float unscaledElevationMercury = shapeGeneratorMercury.CalculateUnscaledElevationMercury(pointOnUnitSphereMercury);

                // verrticies with index of i = point on unit sphere * shape generator with unscaled elevation
                vertices[i] = pointOnUnitSphereMercury * shapeGeneratorMercury.GetScaledElevationMercury(unscaledElevationMercury);

                // uv's on the y axis = unscaled elevation
                uv[i].y = unscaledElevationMercury;
    ...
    // Update UV's
    public void UpdateUVsMercury(ColourGeneratorMercury colourGeneratorMercury) {
        ...
        for (int y = 0; y < resolution; y++) {
            ...
            for (int x = 0; x < resolution; x++) {
                ...
                uv[i].x = colourGeneratorMercury.BiomePercentFromPointMercury(pointOnUnitSphereMercury);
