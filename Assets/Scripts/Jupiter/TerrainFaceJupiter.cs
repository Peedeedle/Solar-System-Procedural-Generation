...
public class TerrainFaceJupiter
{

    // Shape Generator
    ShapeGeneratorJupiter shapeGeneratorJupiter;
    ...
    public TerrainFaceJupiter(ShapeGeneratorJupiter shapeGeneratorJupiter, Mesh mesh, int resolution, Vector3 localUp) {

        // Shape generator
        this.shapeGeneratorJupiter = shapeGeneratorJupiter;
    ...
    public void ConstructMeshJupiter() {
        ...
        for (int y = 0; y < resolution; y++) {
            ...
            for (int x = 0; x < resolution; x++) {
                ...
                Vector3 pointOnUnitCubeJupiter = localUp + (percent.x - 0.5f) * 2 * axisA + (percent.y - 0.5f) * 2 * axisB;

                // vector 3 for point on unit sphere
                Vector3 pointOnUnitSphereJupiter = pointOnUnitCubeJupiter.normalized;

                // Unscaled elevation = shape generator to calculate unscaled elevation with point on unit sphere
                float unscaledElevationJupiter = shapeGeneratorJupiter.CalculateUnscaledElevationJupiter(pointOnUnitSphereJupiter);

                // verrticies with index of i = point on unit sphere * shape generator with unscaled elevation
                vertices[i] = pointOnUnitSphereJupiter * shapeGeneratorJupiter.GetScaledElevationJupiter(unscaledElevationJupiter);

                // uv's on the y axis = unscaled elevation
                uv[i].y = unscaledElevationJupiter;
    ...
    // Update UV's
    public void UpdateUVsJupiter(ColourGeneratorJupiter colourGeneratorJupiter) {
        ...
        for (int y = 0; y < resolution; y++) {
            ...
            for (int x = 0; x < resolution; x++) {
                ...
                uv[i].x = colourGeneratorJupiter.BiomePercentFromPointJupiter(pointOnUnitSphereJupiter);
