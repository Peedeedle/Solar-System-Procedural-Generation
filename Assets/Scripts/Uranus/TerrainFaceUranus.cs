...
public class TerrainFaceUranus {

    // Shape Generator
    ShapeGeneratorUranus shapeGeneratorUranus;
    ...
    public TerrainFaceUranus(ShapeGeneratorUranus shapeGeneratorUranus, Mesh mesh, int resolution, Vector3 localUp) {

        // Shape generator
        this.shapeGeneratorUranus = shapeGeneratorUranus;
    ...
    public void ConstructMeshUranus() {
        ...
        for (int y = 0; y < resolution; y++) {
            ...
            for (int x = 0; x < resolution; x++) {
                ...
                Vector3 pointOnUnitCubeUranus = localUp + (percent.x - 0.5f) * 2 * axisA + (percent.y - 0.5f) * 2 * axisB;

                // vector 3 for point on unit sphere
                Vector3 pointOnUnitSphereUranus = pointOnUnitCubeUranus.normalized;

                // Unscaled elevation = shape generator to calculate unscaled elevation with point on unit sphere
                float unscaledElevationUranus = shapeGeneratorUranus.CalculateUnscaledElevationUranus(pointOnUnitSphereUranus);

                // verrticies with index of i = point on unit sphere * shape generator with unscaled elevation
                vertices[i] = pointOnUnitSphereUranus * shapeGeneratorUranus.GetScaledElevationUranus(unscaledElevationUranus);

                // uv's on the y axis = unscaled elevation
                uv[i].y = unscaledElevationUranus;
    ...
    // Update UV's
    public void UpdateUVsUranus(ColourGeneratorUranus colourGeneratorUranus) {
        ...
        for (int y = 0; y < resolution; y++) {
            ...
            for (int x = 0; x < resolution; x++) {
                ...
                uv[i].x = colourGeneratorUranus.BiomePercentFromPointUranus(pointOnUnitSphereUranus);
